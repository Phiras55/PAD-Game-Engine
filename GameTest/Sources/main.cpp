...
int main()
{
	...
	for (int i = 0; i < 1; ++i)
	{
		...
		cube->GetTransform().SetPosition(pad::math::Vec3f(0, 2, 0));
		cube->GetTransform().SetRotation(pad::math::Vec3f(i * 45, i * 45, i * 45));
		cube->GetTransform().SetScale(0.01f);

		cube->AddComponent<pad::sys::ecs::AnimRenderer>("creature_giantspider_giantspider", "creature_giantspider_giantspider_0", "creature_giantspider_giantspider", "creature_giantspider_giantspider_AttackUnarmed [3]");
